...
export default function WonLostModal({ lead, targetStatus, onConfirm, onCancel }) {
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        >
          {/* Header */}
          <div className={`px-5 py-4 rounded-t-2xl flex items-center justify-between ${isLost ? "bg-red-50 border-b border-red-100" : "bg-emerald-50 border-b border-emerald-100"}`}>
            <div className="flex items-center gap-2.5">
              <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${isLost ? "bg-red-100" : "bg-emerald-100"}`}>
                {isLost ? <AlertTriangle size={16} className="text-red-500" /> : <span className="text-emerald-600 font-bold text-sm">✓</span>}
              </div>
              <div>
            ...
            <button onClick={onCancel} className="p-1.5 rounded-lg hover:bg-white text-[#5C6D71]">
              <X size={16} />
            </button>
          </div>
